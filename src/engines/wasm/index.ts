...
function createCompileFailureResult(errors: Error[]): WasmRunResult | WasmInteractiveRunResult {
  return {
    prints: [],
    rawOutputs: [],
    errors,
...
export async function compileToWasmAndRun(
  ...
  let debugFunctions: NonNullable<BaseWasmRunResult["debugFunctions"]> | null = null;
  ...
  try {
    ...
    if (analysisErrors.length > 0) {
      return createCompileFailureResult(analysisErrors);
    }
      ...
      metacircular: {
        tokenize: (offset: number, length: number) => {
          ...
          return Array.from(pythonLexer)
            ...
            .reduceRight(
              (tail, [tag, value]) => makePair(tag, value, tail[0], tail[1]),
              makeNone(),
            );
        },
    ...
    debugFunctions = {
      ...
      getListElement: (listTag: number, listValue: bigint, index: number) => {
        if (!wasmExports) throw new Error("WASM exports not initialised");
