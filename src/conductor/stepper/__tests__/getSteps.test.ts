...
describe("Python stepper — if/elif and conditional-expression predicates require a strict bool", () => {
  ...
  // truthiness, `if`/`elif`/`x if p else y` all require a genuine `bool` predicate here now —
  // matching the and/or/not treatment just above, and sharing one message ("predicate type must be
  // 'bool', not '<type>'", SICP's term for the condition) between the two constructs instead of
  // "if condition"/"conditional expression condition", phrased like an ordinary operand-type error
  // (`unsupported operand type(s) for -: 'int' and 'str'`). The CSE machine's BRANCH instruction
  // does not enforce this yet (py-slang#436).
  ...
  test("a non-bool conditional-expression condition is a TypeError (stuck), not truthy-evaluated", async () => {
    ...
    expect(await result("1 if 5 else 2")).toBe(
      "TypeError: predicate type must be 'bool', not 'int'",
    );
    expect(await result("1 if None else 2")).toBe(
      "TypeError: predicate type must be 'bool', not 'NoneType'",
    );
  });
...
describe("Python stepper — real module resolution (py-slang#385)", () => {
  ...
  test("a module function's returned closure is module-generated, not the import itself (py-slang#407)", async () => {
    ...
    // `from X import Y` statement of its own.
    async function* innerFunc(
      a: TypedValue<DataType>,
    ): AsyncGenerator<void, TypedValue<DataType>, undefined> {
      await Promise.resolve();
      return { type: DataType.NUMBER, value: (a as TypedValue<DataType.NUMBER>).value + 1 };
    }
    async function* sineWaveFunc(): AsyncGenerator<void, TypedValue<DataType.CLOSURE>, undefined> {
      await Promise.resolve();
      return dh.closure_make({ returnType: DataType.NUMBER, args: [DataType.NUMBER] }, innerFunc);
    }
    const sineWave = await dh.closure_make(
      { returnType: DataType.CLOSURE, args: [] },
      sineWaveFunc,
    );
    installFakeModule({ rune: [{ symbol: "sine_wave", value: sineWave }] });

    const ast = parse("from rune import sine_wave\nx = sine_wave()\nx\n");
    const stepped = await getPythonSteps(ast, undefined, { evaluator: dh });
    const withGenerated = stepped.find(s =>
      findNode(s.ast, (n: any) => n.type === "ModuleFunction" && n.name === "_from_sine_wave"),
    );
    expect(withGenerated).toBeDefined();
    const node = findNode(
      withGenerated!.ast,
      (n: any) => n.type === "ModuleFunction" && n.name === "_from_sine_wave",
    );
    expect(node).toMatchObject({
      name: "_from_sine_wave",
      hoverText: "function generated by module function sine_wave",
    });
  });

  test("a call returning several closures at once gets each disambiguated, same hover text (py-slang#407)", async () => {
    const dh = new GenericDataHandler(2);
    // make_sound() returns ((wave1, wave2), duration) — two fresh closures nested inside the same
    // result, like rune's Sound. Both came from the same make_sound() call, so they'd render
    // identically as `_from_make_sound` with nothing to tell them apart on screen; each needs its own
    // `_1`/`_2` suffix, while the hover popover (naming the generator, not the individual value) stays
    // the same for both.
    async function* wave1Func(
      a: TypedValue<DataType>,
    ): AsyncGenerator<void, TypedValue<DataType>, undefined> {
      await Promise.resolve();
      return { type: DataType.NUMBER, value: (a as TypedValue<DataType.NUMBER>).value };
    }
    async function* wave2Func(
      a: TypedValue<DataType>,
    ): AsyncGenerator<void, TypedValue<DataType>, undefined> {
      await Promise.resolve();
      return { type: DataType.NUMBER, value: -(a as TypedValue<DataType.NUMBER>).value };
    }
    async function* makeSoundFunc(): AsyncGenerator<void, TypedValue<DataType>, undefined> {
      await Promise.resolve();
      const f1 = await dh.closure_make(
        { returnType: DataType.NUMBER, args: [DataType.NUMBER] },
        wave1Func,
      );
      const f2 = await dh.closure_make(
        { returnType: DataType.NUMBER, args: [DataType.NUMBER] },
        wave2Func,
      );
      const waves = await dh.pair_make(f1, f2);
      return dh.pair_make(waves, { type: DataType.NUMBER, value: 4 });
    }
    const makeSound = await dh.closure_make({ returnType: DataType.PAIR, args: [] }, makeSoundFunc);
    installFakeModule({ sound: [{ symbol: "make_sound", value: makeSound }] });

    const ast = parse("from sound import make_sound\nx = make_sound()\nx\n");
    const stepped = await getPythonSteps(ast, undefined, { evaluator: dh });
    const withResult = stepped.find(s => findNode(s.ast, (n: any) => n.type === "ArrayExpression"));
    expect(withResult).toBeDefined();
    const outer = findNode(withResult!.ast, (n: any) => n.type === "ArrayExpression");
    const [wavesNode, durationNode] = outer.elements as any[];
    const [f1Node, f2Node] = wavesNode.elements as any[];
    expect(f1Node).toMatchObject({
      type: "ModuleFunction",
      name: "_from_make_sound_1",
      hoverText: "function generated by module function make_sound",
    });
    expect(f2Node).toMatchObject({
      type: "ModuleFunction",
      name: "_from_make_sound_2",
      hoverText: "function generated by module function make_sound",
    });
    expect(durationNode).toMatchObject({ type: "Literal", value: 4 });
  });
