...
export class BuiltInFunctions {
    ...
    static _int(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        ...
        if (args.length === 1) {
            ...
            if (base < 2 || base > 36) {
                handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "float' or 'int"))
            }
        ...
        handleRuntimeError(
            context,
            new TypeError(source, command as ExprNS.Expr, context, arg.type, 'string, a bytes-like object or a real number')
        )
    }

    @Validate(1, 2, '_int_from_string', true)
    static _int_from_string(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        ...
        if (args.length === 2) {
            ...
            if (baseVal.type === 'bigint') {
                ...
                handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, args[1].type, "float' or 'int"));
            }
        }

        // base should be in between 2 and 36
        if (base < 2 || base > 36) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "_int_from_string"));
        }
    ...
    static abs(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue | NumberValue {
        const x = args[0];
        switch (x.type) {
            case 'bigint': {
            ...
            default:
                handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, args[0].type, "float', 'int' or 'complex"));
        }
    }
        ...
        if (classinfo.type === 'string') {
            switch (classinfo.value) {
                case 'int':
                    expectedType = 'bigint';
                    if (obj.type === 'bool') {
                        handleRuntimeError(context, new SublanguageError(source, command as ExprNS.Expr, context,
                            'isinstance',
                            '1',
    ...
    static math_acos(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        ...
        if (num < -1 || num > 1) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_acos"));
        }
    ...
    static math_acosh(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        ...
        if (num < 1) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_acosh"));
        }
    ...
    static math_asin(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
        ...
        if (num < -1 || num > 1) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_asin"));
        }
    ...
    static math_asinh(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_atan(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_atan2(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const y = args[0];
        const x = args[1];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
    ...
    static math_atanh(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
        ...
        if (num <= -1 || num >= 1) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_atanh"));
        }
    ...
    static math_cos(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_cosh(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_degrees(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_erf(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static char_at(args: Value[], source: string, command: ControlItem, context: Context): StringValue {
        const s = args[0];
        const i = args[1];

        if (s.type !== 'string') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, s.type, "string"));
        }
    ...
    static math_comb(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        const n = args[0];
        const k = args[1];

        if (n.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, n.type, "int"));
        } else if (k.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, k.type, "int"));
        }
        ...
        if (nVal < 0 || kVal < 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_comb"));
        }
    ...
    static math_factorial(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        const n = args[0];
        ...
        if (nVal < 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_factorial"));
        }
    ...
    static math_gcd(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        if (args.length === 0) {
            return { type: 'bigint', value: BigInt(0) };
        }
        ...
        const values = args.map((v, idx) => {
            if (v.type !== 'bigint') {
                handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, v.type, "int"));
            }
    ...
    static math_isqrt(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        const nValObj = args[0];
        if (nValObj.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, nValObj.type, "int"));
        }

        const n: bigint = nValObj.value;

        if (n < 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_isqrt"));
        }
    ...
    static math_lcm(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        if (args.length === 0) {
            return { type: 'bigint', value: BigInt(1) };
        }
        ...
        const values = args.map((val, idx) => {
            if (val.type !== 'bigint') {
                handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, val.type, "int"));
            }
    ...
    static math_perm(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        const nValObj = args[0];
        if (nValObj.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, nValObj.type, "int"));
        }
        ...
        if (args.length === 2) {
            ...
            if (kValObj.type === 'none') {
                ...
                handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, kValObj.type, "int' or 'None"));
            }
        }

        if (n < 0 || k < 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_perm"));
        }
    ...
    static math_ceil(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        const x = args[0];
    ...
    static math_fabs(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
    ...
    static math_floor(args: Value[], source: string, command: ControlItem, context: Context): BigIntValue {
        const x = args[0];
    ...
    static toNumber(val: Value, source: string, command: ControlItem, context: Context): number {
        if (val.type === 'bigint') {
            ...
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, val.type, "float' or 'int"));
            return 0;
        }
    }

    @Validate(3, 3, 'math_fma', false)
    static math_fma(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
    ...
    static math_fmod(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        ...
        if (yVal === 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_fmod"));
        }
    ...
    static math_remainder(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        const y = args[1];
        ...
        if (yValue === 0) {
            ...
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_remainder"));
        }
    ...
    static math_copysign(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const [x, y] = args;
    ...
    static math_isfinite(args: Value[], source: string, command: ControlItem, context: Context): BoolValue {
        const xValObj = args[0];
        if (xValObj.type !== 'number' && xValObj.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, xValObj.type, "float' or 'int"));
        }
    ...
    static math_isinf(args: Value[], source: string, command: ControlItem, context: Context): BoolValue {
        const xValObj = args[0];
        if (xValObj.type !== 'number' && xValObj.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, xValObj.type, "float' or 'int"));
        }
    ...
    static math_isnan(args: Value[], source: string, command: ControlItem, context: Context): BoolValue {
        const xValObj = args[0];
        if (xValObj.type !== 'number' && xValObj.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, xValObj.type, "float' or 'int"));
        }
    ...
    static math_ldexp(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const xVal = BuiltInFunctions.toNumber(args[0], source, command, context);
    ...
    static math_nextafter(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
    ...
    static math_cbrt(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const xVal = args[0];
        let x: number;
    ...
    static math_exp(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const xVal = args[0];
        let x: number;
    ...
    static math_exp2(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const xVal = args[0];
        let x: number;
    ...
    static math_expm1(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_gamma(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_lgamma(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_log(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
        ...
        if (num <= 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_log"));
        }
        ...
        if (baseArg.type !== 'number' && baseArg.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, baseArg.type, "float' or 'int"));
        }
        let baseNum: number;
    ...
    static math_log10(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
        ...
        if (num <= 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_log10"));
        }
    ...
    static math_log1p(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, args[0].type, "float' or 'int"));
        }
        ...
        if (1 + num <= 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_log1p"));
        }
    ...
    static math_log2(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, args[0].type, "float' or 'int"));
        }
        ...
        if (num <= 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_log2"));
        }
    ...
    static math_pow(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const base = args[0];
        const exp = args[1];

        if (base.type !== 'number' && base.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, base.type, "float' or 'int"));
        } else if (exp.type !== 'number' && exp.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, exp.type, "float' or 'int"));
        }
    ...
    static math_radians(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_tanh(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
    ...
    static math_sqrt(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const x = args[0];
        if (x.type !== 'number' && x.type !== 'bigint') {
            handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, x.type, "float' or 'int"));
        }
        ...
        if (num < 0) {
            handleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "math_sqrt"));
        }
    ...
    static max(args: Value[], source: string, command: ControlItem, context: Context): Value {
        ...
        for (let i = 1; i < args.length; i++) {
            ...
            if (isNumeric && !numericTypes.includes(t)) {
                handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, args[i].type, "float' or 'int"));
            }
            if (isString && t !== 'string') {
                handleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, args[i].type, "string"));
            }
        }
    ...
    static min(args: Value[], source: string, command: ControlItem, context: Context): Value {
        if (args.length < 2) {
            handleRuntimeError(context, new MissingRequiredPositionalError(source, (command as ExprNS.Expr), 'min', Number(2), args, true));
        }
    ...
    static random_random(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
        const result = Math.random();
        return { type: 'number', value: result };
    }

    @Validate(1, 2, 'round', true)
    static round(args: Value[], source: string, command: ControlItem, context: Context): NumberValue | BigIntValue {
        const numArg = args[0];
    ...
    static time_time(args: Value[], source: string, command: ControlItem, context: Context): NumberValue {
