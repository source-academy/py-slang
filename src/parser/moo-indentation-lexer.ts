...
class IndentationLexer implements moo.Lexer {
  ...
  private _isNewline(token: moo.Token | undefined): boolean {
    return (
      !!token &&
      (this._newlineType === null ? token.text.endsWith("\n") : token.type === this._newlineType)
    );
  }
  ...
  private _next(): IndentationToken | undefined {
    ...
    if (this._state === "lineContent") {
      ...
      const indentation = (this._queuedLines[this._queuedLines.length - 1] || [])
        .map(({ text }) => text)
        .join("");
  ...
  has(tokenType: string): boolean {
    return (
      tokenType === this._indentName || tokenType === this._dedentName || this._lexer.has(tokenType)
    );
  }
}
