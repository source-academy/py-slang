...
import { PyControlItem } from './cse-machine/py_control'
...
export function Validate(
  ...
  return function (
    ...
    descriptor: TypedPropertyDescriptor<
      (args: Value[], source: string, command: PyControlItem, context: PyContext) => Value
    >
  ): void {
...
export class BuiltInFunctions {
  static print(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
  ...
  static _int(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    if (args.length === 0) {
    ...
    if (args.length === 1) {
      ...
      if (arg.type === 'string') {
        ...
        if (!/^[+-]?\d+$/.test(str)) {
          pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'int'))
        }
      ...
      if (arg.type !== 'string') {
        pyHandleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, arg.type, 'string'))
      }
      ...
      if (base < 2 || base > 36) {
        pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, "float' or 'int"))
      }
  ...
  static abs(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    switch (x.type) {
      case 'bigint': {
      ...
      default:
        pyHandleRuntimeError(
          context,
          new TypeError(source, command as ExprNS.Expr, context, args[0].type, "float', 'int' or 'complex")
        )
    }
  }

  @Validate(0, 1, 'str', false)
  static str(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    if (args.length === 0) {
  ...
  @Validate(1, null, 'error', false)
  static error(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const output = 'Error: ' + args.map(arg => toPythonString(arg)).join(' ') + '\n'
    throw new Error(output)
  }

  @Validate(2, 2, 'isinstance', false)
  static isinstance(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const obj = args[0]
  ...
  static math_acos(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    ...
    if (num < -1 || num > 1) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_acos'))
    }
  ...
  static math_acosh(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    ...
    if (num < 1) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_acosh'))
    }
  ...
  static math_asin(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
    ...
    if (num < -1 || num > 1) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_asin'))
    }
  ...
  static math_asinh(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_atan(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_atan2(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const y = args[0]
    const x = args[1]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
  ...
  static math_atanh(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
    ...
    if (num <= -1 || num >= 1) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_atanh'))
    }
  ...
  static math_cos(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_cosh(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_degrees(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_erf(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_erfc(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static char_at(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const s = args[0]
    const i = args[1]
  ...
  static math_comb(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const n = args[0]
    const k = args[1]

    if (n.type !== 'bigint') {
      pyHandleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, n.type, 'int'))
    } else if (k.type !== 'bigint') {
      pyHandleRuntimeError(context, new TypeError(source, command as ExprNS.Expr, context, k.type, 'int'))
    }
    ...
    if (nVal < 0 || kVal < 0) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_comb'))
    }
  ...
  static math_factorial(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const n = args[0]
  ...
  static math_gcd(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    if (args.length === 0) {
      return { type: 'bigint', value: BigInt(0) }
    }
  ...
  static math_isqrt(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    ...
    if (n < 0) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_isqrt'))
    }
  ...
  static math_lcm(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    if (args.length === 0) {
      return { type: 'bigint', value: BigInt(1) }
    }
  ...
  static math_perm(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const nValObj = args[0]
    if (nValObj.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
    ...
    if (n < 0 || k < 0) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_perm'))
    }
  ...
  static math_ceil(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
  ...
  static math_fabs(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
  ...
  static math_floor(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
  ...
  static toNumber(val: Value, source: string, command: PyControlItem, context: PyContext): number {
    if (val.type === 'bigint') {
  ...
  static math_fma(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
  ...
  static math_fmod(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    ...
    if (yVal === 0) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_fmod'))
    }
  ...
  static math_trunc(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
  ...
  static math_isinf(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const xValObj = args[0]
    if (xValObj.type !== 'number' && xValObj.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_isnan(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const xValObj = args[0]
    if (xValObj.type !== 'number' && xValObj.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_ldexp(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const xVal = BuiltInFunctions.toNumber(args[0], source, command, context)
  ...
  static math_ulp(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
  ...
  static math_cbrt(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const xVal = args[0]
    let x: number
  ...
  static math_exp(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const xVal = args[0]
    let x: number
  ...
  static math_exp2(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const xVal = args[0]
    let x: number
  ...
  static math_expm1(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_gamma(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_lgamma(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_log(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
    ...
    if (num <= 0) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_log'))
    }
  ...
  static math_log10(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
    ...
    if (num <= 0) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_log10'))
    }
  ...
  static math_log1p(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
    ...
    if (1 + num <= 0) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_log1p'))
    }
  ...
  static math_log2(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
    ...
    if (num <= 0) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_log2'))
    }
  ...
  static math_pow(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const base = args[0]
    const exp = args[1]
  ...
  static math_sin(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_sinh(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_tan(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_tanh(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
  ...
  static math_sqrt(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const x = args[0]
    if (x.type !== 'number' && x.type !== 'bigint') {
      pyHandleRuntimeError(
        context,
    ...
    if (num < 0) {
      pyHandleRuntimeError(context, new ValueError(source, command as ExprNS.Expr, context, 'math_sqrt'))
    }
  ...
  static max(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
  ...
  static min(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    if (args.length < 2) {
      pyHandleRuntimeError(
        context,
        new MissingRequiredPositionalError(source, command as ExprNS.Expr, 'min', Number(2), args, true)
      )
    }
  ...
  static round(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const numArg = args[0]
  ...
  static time_time(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
    const currentTime = Date.now()
    return { type: 'number', value: currentTime }
  }

  static input(args: Value[], source: string, command: PyControlItem, context: PyContext): Value {
