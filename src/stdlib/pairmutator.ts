...
class PairmutatorBuiltins {
    ...
    static set_head(args: Value[], source: string, command: ControlItem, context: Context): NoneValue {
        const head = args[0];
        const tail = args[1];
    ...
    static set_tail(args: Value[], source: string, command: ControlItem, context: Context): NoneValue {
        const head = args[0];
        const tail = args[1];
        if (head.type !== 'list' || head.value.length !== 2) {
...
for (const builtin of Object.getOwnPropertyNames(PairmutatorBuiltins)) {
    if (typeof PairmutatorBuiltins[builtin as keyof typeof PairmutatorBuiltins] === 'function' && !builtin.startsWith('_')) {
        pairmutatorBuiltins.set(builtin, { type: 'builtin', func: PairmutatorBuiltins[builtin as keyof typeof PairmutatorBuiltins] as any, name: builtin }); // TODO: fix typing
    }
}
export default {
