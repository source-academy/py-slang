// tags
const INT_TAG = "$_int_tag";
const FLOAT_TAG = "$_float_tag";
const COMPLEX_TAG = "$_complex_tag";
const BOOL_TAG = "$_bool_tag";
const STRING_TAG = "$_string_tag";

export const HEAP_PTR = "$_heap_pointer";

// globals
export const GLOBALS = [
  `(global ${INT_TAG} i32 (i32.const 0))`,
  `(global ${FLOAT_TAG} i32 (i32.const 1))`,
  `(global ${COMPLEX_TAG} i32 (i32.const 2))`,
  `(global ${BOOL_TAG} i32 (i32.const 3))`,
  `(global ${STRING_TAG} i32 (i32.const 4))`,
];
...
const makeIntFunc = `(func ${MAKE_INT_FX} (param $value i64) (result i32 i64) (global.get ${INT_TAG}) (local.get $value))`;
const makeFloatFunc = `(func ${MAKE_FLOAT_FX} (param $value f64) (result i32 i64) (global.get ${FLOAT_TAG}) (local.get $value) (i64.reinterpret_f64))`;
const makeComplexFunc = `(func ${MAKE_COMPLEX_FX} (param $real f64) (param $img f64) (result i32 i64) (global.get ${HEAP_PTR}) (local.get $real) (f64.store) (global.get ${HEAP_PTR}) (i32.const 8) (i32.add) (local.get $img) (f64.store) (global.get ${COMPLEX_TAG}) (global.get ${HEAP_PTR}) (i64.extend_i32_u) (global.get ${HEAP_PTR}) (i32.const 16) (i32.add) (global.set ${HEAP_PTR}))`;
const makeBoolFunc = `(func ${MAKE_BOOL_FX} (param $value i32) (result i32 i64) (global.get ${BOOL_TAG}) (local.get $value) (i64.extend_i32_u))`;
const makeStringFunc = `(func ${MAKE_STRING_FX} (param $ptr i32) (param $len i32) (result i32 i64) (global.get ${STRING_TAG}) (local.get $ptr) (i64.extend_i32_u) (i64.const 32) (i64.shl) (local.get $len) (i64.extend_i32_u) (i64.or))`;
...
export const LOG_FUNCS = [
  ...
  `(func $log (param $tag i32) (param $value i64)
    (local.get $tag) (global.get ${INT_TAG}) i32.eq (if
      (then (local.get $value) (call $_log_int) return))
    (local.get $tag) (global.get ${FLOAT_TAG}) i32.eq (if
      (then (local.get $value) (f64.reinterpret_i64) (call $_log_float) return))
    (local.get $tag) (global.get ${COMPLEX_TAG}) i32.eq (if
    ...
    (local.get $tag) (global.get ${BOOL_TAG}) i32.eq (if
      (then (local.get $value) (call $_log_bool) return))
    (local.get $tag) (global.get ${STRING_TAG}) i32.eq (if
...
const binaryOpFactory = (
  ...
  ${
    name === ADD_FX
      ? `(i32.eq (local.get $x_tag) (global.get ${STRING_TAG})) (i32.eq (local.get $y_tag) (global.get ${STRING_TAG})) (i32.and) (if` +
  ...
  (i32.eq (local.get $x_tag) (global.get ${BOOL_TAG})) (if (then (global.get ${INT_TAG}) (local.set $x_tag)))
  (i32.eq (local.get $y_tag) (global.get ${BOOL_TAG})) (if (then (global.get ${INT_TAG}) (local.set $y_tag)))

  ${
    /* if both int, use intOperation (except for division: use floatOperation) */ ""
  }
  (i32.eq (local.get $x_tag) (global.get ${INT_TAG})) (i32.eq (local.get $y_tag) (global.get ${INT_TAG})) (i32.and) (if
    ${
      name === DIV_FX
  ...
  (i32.eq (local.get $x_tag) (global.get ${INT_TAG})) (if (result f64)
    (then (local.get $x_val) (f64.convert_i64_s) (global.get ${FLOAT_TAG}) (local.set $x_tag))
    (else (local.get $x_val) (f64.reinterpret_i64))
  )
  (local.set $a)
  (i32.eq (local.get $y_tag) (global.get ${INT_TAG})) (if (result f64)
    (then (local.get $y_val) (f64.convert_i64_s) (global.get ${FLOAT_TAG}) (local.set $y_tag))
    (else (local.get $y_val) (f64.reinterpret_i64))
  )
  (local.set $c)

  ${/* if both float, use floatOperation */ ""}
  (i32.eq (local.get $x_tag) (global.get ${FLOAT_TAG})) (i32.eq (local.get $y_tag) (global.get ${FLOAT_TAG})) (i32.and) (if
  ...
  (i32.eq (local.get $x_tag) (global.get ${FLOAT_TAG})) (if
    (then (global.get ${COMPLEX_TAG}) (local.set $x_tag))
  ...
  (i32.eq (local.get $y_tag) (global.get ${FLOAT_TAG})) (if
    (then (global.get ${COMPLEX_TAG}) (local.set $y_tag))
  ...
  (i32.eq (local.get $x_tag) (global.get ${COMPLEX_TAG})) (i32.eq (local.get $y_tag) (global.get ${COMPLEX_TAG})) (i32.and) (if
    (then
     ${
