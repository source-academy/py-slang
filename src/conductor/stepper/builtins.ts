...
const MATH_BINARY_FLOAT: Record<string, (x: number, y: number) => number> = {
  ...
  math_copysign: (x: number, y: number) =>
    (Object.is(y, -0) || y < 0 ? -1 : 1) * Math.abs(x),
...
Object.assign(BUILTIN_FUNCTIONS, {
  ...
  abs: (args: StepNode[]): StepNode => {
    ...
    if (isFloatNode(x) || isBoolNode(x)) return numberLiteral(Math.abs(asNumber(x, 'abs')), isFloatNode(x));
  ...
  float: (args: StepNode[]): StepNode => {
    ...
    if (isStrNode(x)) {
      ...
      const specials: Record<string, number> = { inf: Infinity, '-inf': -Infinity, infinity: Infinity, '-infinity': -Infinity, nan: NaN };
  ...
  round: (args: StepNode[]): StepNode => {
    ...
    const x = args[0];
    const ndigits = args.length === 2 && !isNoneNode(args[1]) ? Number(asNumber(args[1], 'round')) : null;
