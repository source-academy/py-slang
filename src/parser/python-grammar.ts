...
let ParserRules = [
    ...
    {"name": "simple_stmt", "symbols": ["small_stmt", "_", (pythonLexer.has("newline") ? {type: "newline"} : newline)], "postprocess": id},
    ...
    {"name": "assign_stmt", "symbols": [(pythonLexer.has("identifier") ? {type: "identifier"} : identifier), "_", {"literal":":"}, "_", "test", "_", {"literal":"="}, "_", "test"], "postprocess": ([n,,,, ann,,,, v]) => { const tok = toAstToken(n); return new StmtNS.AnnAssign(tok, v.endToken, new ExprNS.Variable(tok, tok, tok), v, ann); }},
    ...
    {"name": "assign_stmt", "symbols": [(pythonLexer.has("identifier") ? {type: "identifier"} : identifier), "_", {"literal":"="}, "_", "test"], "postprocess": ([n,,,, v]) => { const tok = toAstToken(n); return new StmtNS.Assign(tok, v.endToken, new ExprNS.Variable(tok, tok, tok), v); }},
    ...
    {"name": "suite_stmts", "symbols": ["suite_stmts", "_", (pythonLexer.has("newline") ? {type: "newline"} : newline)], "postprocess": id},
    ...
    {"name": "or_test", "symbols": ["or_test", "_", (pythonLexer.has("kw_or") ? {type: "kw_or"} : kw_or), "_", "and_test"], "postprocess": ([left,, op,, right]) => new ExprNS.BoolOp(left.startToken, right.endToken, left, toAstToken(op), right)},
    {"name": "or_test", "symbols": ["and_test"], "postprocess": id},
    {"name": "and_test", "symbols": ["and_test", "_", (pythonLexer.has("kw_and") ? {type: "kw_and"} : kw_and), "_", "not_test"], "postprocess": ([left,, op,, right]) => new ExprNS.BoolOp(left.startToken, right.endToken, left, toAstToken(op), right)},
    ...
    {"name": "comparison", "symbols": ["comparison", "_", "comp_op", "_", "arith"], "postprocess": ([left,, op,, right]) => new ExprNS.Compare(left.startToken, right.endToken, left, op, right)},
    ...
    {"name": "comp_op", "symbols": [(pythonLexer.has("kw_not") ? {type: "kw_not"} : kw_not), "_", (pythonLexer.has("kw_in") ? {type: "kw_in"} : kw_in)], "postprocess": ([t,,]) => { const tok = toAstToken(t); tok.lexeme = 'not in'; return tok; }},
    {"name": "comp_op", "symbols": [(pythonLexer.has("kw_is") ? {type: "kw_is"} : kw_is)], "postprocess": ([t]) => toAstToken(t)},
    {"name": "comp_op", "symbols": [(pythonLexer.has("kw_is") ? {type: "kw_is"} : kw_is), "_", (pythonLexer.has("kw_not") ? {type: "kw_not"} : kw_not)], "postprocess": ([t,,]) => { const tok = toAstToken(t); tok.lexeme = 'is not'; return tok; }},
    {"name": "arith", "symbols": ["arith", "_", "arith_op", "_", "term"], "postprocess": ([left,, op,, right]) => new ExprNS.Binary(left.startToken, right.endToken, left, op, right)},
    ...
    {"name": "term", "symbols": ["term", "_", "term_op", "_", "factor"], "postprocess": ([left,, op,, right]) => new ExprNS.Binary(left.startToken, right.endToken, left, op, right)},
    ...
    {"name": "string", "symbols": [(pythonLexer.has("stringTripleDouble") ? {type: "stringTripleDouble"} : stringTripleDouble)], "postprocess": ([t]) => { const tok = toAstToken(t); return new ExprNS.Literal(tok, tok, stripQuotes(t.value)); }},
    {"name": "string", "symbols": [(pythonLexer.has("stringTripleSingle") ? {type: "stringTripleSingle"} : stringTripleSingle)], "postprocess": ([t]) => { const tok = toAstToken(t); return new ExprNS.Literal(tok, tok, stripQuotes(t.value)); }},
    {"name": "string", "symbols": [(pythonLexer.has("stringDouble") ? {type: "stringDouble"} : stringDouble)], "postprocess": ([t]) => { const tok = toAstToken(t); return new ExprNS.Literal(tok, tok, stripQuotes(t.value)); }},
    {"name": "string", "symbols": [(pythonLexer.has("stringSingle") ? {type: "stringSingle"} : stringSingle)], "postprocess": ([t]) => { const tok = toAstToken(t); return new ExprNS.Literal(tok, tok, stripQuotes(t.value)); }},
    {"name": "_", "symbols": []},
